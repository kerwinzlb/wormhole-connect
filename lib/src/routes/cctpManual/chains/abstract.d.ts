...
export default interface ManualCCTP<T = any> {
    send(token: TokenId | 'native', amount: string, sendingChain: ChainName | ChainId, senderAddress: string, recipientChain: ChainName | ChainId, recipientAddress: string): Promise<T>;
    redeem(destChain: ChainName | ChainId, message: SignedMessage, payer: string): Promise<T>;
